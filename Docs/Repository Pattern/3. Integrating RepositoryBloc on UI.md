# Integrating RepositoryBloc on UI:
...
```dart
    MaterialApp(
      ...
      home: RepositoryProvider(
        create: (context) => UserRepository(UserProvider()), // initialize repository with provider
        child: BlocProvider(
          create: (context) => UserBloc(context.read<UserRepository>()), // injecting repository into bloc
          child: MyHomePage()
        ),
      )
    );
